...
class IntiativeResult extends Character {
  constructor(characterID) {
    super(characterID)
    this.total = null
  }
...
class InitiativeExpression extends Character {
  constructor(characterID, tokenId) {
    super(characterID)
    this.modifier = parseInt(null)
    this.dice = null
    this.expression = undefined
    this.markers = findObjs({_id: tokenId, _type: "graphic"})[0].attributes['statusmarkers']
    this.woundToggle = getAttrByName(characterID, `wounds_toggle`) != 0 ? true : false
    this.wounds = parseInt(getAttrByName(characterID, `wounds`)) || 0
  }
}
...
var sr5api = sr5api || (function() {
    ...
    if (args[0] === "!sr5" && msg.type === "api") {
        ...
        switch(args[1]) {
          ...
          case "rollInit":
              ...
              break;
          default:
              //This brings up the UI menu if the user typed !sr5 without correct arguements
              apiMenu(who)
        };
  ...
  */
  ...
  processResults = results => {
    try {
      ...
      results.forEach(roll =>{
        ...
        const character = new IntiativeResult(characterID)
   ...
   * @return: [] of objects with expressions to roll
   */
  findInitiativeScores = selected => {
    try {
      ...
      selected.forEach(token => {
        ...
        const character = new InitiativeExpression(characterID, token["_id"]);
        //  use the character.types and/or the markers on the Token to determine what
        //  sort of initiative to call: 'matrix', 'astral' or 'initiative'; default is
        //  just 'initiative'
        const matrix = ['sprite', 'host', 'vehicle']
        const intiativeType =
